...
- [ ] T007 [P] [US1] Modify `complete_sync` in `agentix-llama/src/lib.rs` to conditionally rebuild the sampler chain with grammar first: if `req.grammar` is `Some(GrammarConstraint::Gbnf(gbnf))`, build `LlamaSampler::grammar(&model, &gbnf, "root").map_err(|e| InferError::Backend(format!("grammar init: {e:?}")))` and then call `LlamaSampler::chain_simple([grammar_sampler, LlamaSampler::top_k(40), LlamaSampler::top_p(...), LlamaSampler::temp(...), LlamaSampler::dist(...)])` — build a new chain from scratch with grammar as the first element (NOT appended to an existing chain); if `req.grammar` is `None`, build the existing chain unchanged
